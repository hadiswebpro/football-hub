...
function createLeagueCard(competition) {
    ...
    card.innerHTML = `
        <button class="league-card__favorite ${isFavorite ? "is-favorite" : ""}" type="button" aria-label="${isFavorite ? "Remove" : "Add"} ${competition.league.name} ${isFavorite ? "from" : "to"} favorites" aria-pressed="${isFavorite}">
            <span aria-hidden="true">★</span>
        </button>
        <div class="league-card__topline">
            <span class="league-card__number">#${competition.league.id}</span>
            <span class="league-card__status league-card__status--${status.toLowerCase()}"><i></i>${status}</span>
        </div>
        ...
        <div class="league-info">
            ...
            <p class="league-card__country">${competition.country?.name ?? competition.country ?? "International"}</p>
            <span class="league-card__type">${competition.league.type ?? "League"}</span>
            ${currentSeason ? `<div class="league-card__dates"><span>${formatDate(currentSeason.start)}</span><b>—</b><span>${formatDate(currentSeason.end)}</span></div>` : ""}
        </div>
        <div class="league-card__footer"><span>View league</span><span class="league-card__arrow" aria-hidden="true">↗</span></div>
    `;
    ...
    favoriteButton.addEventListener("click", (event) => {
        ...
        favoriteButton.setAttribute("aria-label", `${active ? "Remove" : "Add"} ${competition.league.name} ${active ? "from" : "to"} favorites`);
    });
    ...
    const openCard = () => {
        ...
        const selectedLeague = {
            ...
            seasons: Array.isArray(competition.seasons) ? competition.seasons : []
        };
        sessionStorage.setItem(SELECTED_LEAGUE_KEY, JSON.stringify(selectedLeague));
        localStorage.setItem(SELECTED_LEAGUE_BACKUP_KEY, JSON.stringify(selectedLeague));
