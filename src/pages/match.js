...
function getSelectedMatch() {
    try { return JSON.parse(sessionStorage.getItem("football-hub-selected-match")); }
    catch { return null; }
}
...
function createEventRows(events) {
    ...
    return `<div class="match-page__event-list">${events.map((event) => `<div class="match-page__event"><strong>${event.time?.elapsed ?? "–"}'${event.time?.extra ? `+${event.time.extra}` : ""}</strong><span class="match-page__event-icon">${event.type === "Card" ? "▰" : event.type === "subst" ? "↕" : "⚽"}</span><div><b>${event.player?.name ?? event.type}</b><small>${event.detail ?? ""}${event.team?.name ? ` · ${event.team.name}` : ""}</small></div></div>`).join("")}</div>`;
}
...
function createStatsRows(statistics, homeName, awayName) {
    ...
    return `<div class="match-page__stats"><div class="match-page__stats-head"><span>${homeName}</span><b>STATISTICS</b><span>${awayName}</span></div>${[...values.entries()].map(([name, [home, away]]) => `<div class="match-page__stat"><strong>${home ?? "–"}</strong><div><span>${name}</span><div class="match-page__stat-bar"><i style="width:${typeof home === "number" && typeof away === "number" && home + away ? (home / (home + away)) * 100 : 50}%"></i></div></div><strong>${away ?? "–"}</strong></div>`).join("")}</div>`;
}
...
function createLineupRows(lineups) {
    ...
    return `<div class="match-page__lineups">${lineups.map((team) => `<div class="match-page__lineup"><div class="match-page__lineup-head"><img src="${team.team?.logo ?? ""}" alt="${team.team?.name ?? "Team"} logo"><div><h3>${team.team?.name ?? "Team"}</h3><span>${team.formation ?? "Formation unavailable"}</span></div></div><div class="match-page__players">${(team.startXI ?? []).map(({ player }) => `<span><b>${player.number ?? ""}</b>${player.name}</span>`).join("")}</div></div>`).join("")}</div>`;
}
...
function createMatchPage() {
    ...
    app.innerHTML = `<section class="match-page">
        ...
        <section class="match-page__content match-page__content--active" data-content="overview">
            ...
            <div class="match-page__panel"><span class="match-page__panel-label">KICK-OFF</span><h2 data-meta="date">${formatDate(match.date)}</h2><p data-meta="time">${formatTime(match.date)} · Asia/Tehran</p></div>
            <div class="match-page__panel"><span class="match-page__panel-label">VENUE</span><h2 data-meta="venue">Loading venue…</h2><p data-meta="city">Match venue information</p></div>
            <div class="match-page__panel"><span class="match-page__panel-label">MATCH INFO</span><h2 data-meta="round">${match.round ?? "Fixture details"}</h2><p data-meta="referee">Referee: Loading…</p></div>
        </section>
    ...
    app.querySelector(".match-page__back").addEventListener("click", () => { sessionStorage.removeItem("football-hub-selected-match"); window.location.hash = "#matches"; });
    ...
    const loadTab = async (panel) => {
        ...
        try {
            if (panel === "events") target.innerHTML = `<div class="match-page__panel match-page__panel--wide"><span class="match-page__panel-label">TIMELINE</span><h2>Match Events</h2>${createEventRows(await getFixtureEvents(match.fixtureId))}</div>`;
            if (panel === "stats") { const data = await getFixtureStatistics(match.fixtureId); target.innerHTML = `<div class="match-page__panel match-page__panel--wide"><span class="match-page__panel-label">PERFORMANCE</span><h2>Match Statistics</h2>${createStatsRows(data, match.home.name, match.away.name)}</div>`; }
            if (panel === "lineups") target.innerHTML = `<div class="match-page__panel match-page__panel--wide"><span class="match-page__panel-label">SQUADS</span><h2>Starting Lineups</h2>${createLineupRows(await getFixtureLineups(match.fixtureId))}</div>`;
            loaded.add(panel);
        } catch { target.innerHTML = `<div class="match-page__empty match-page__empty--small"><span>!</span><h2>Could not load data</h2><p>Please check your API key and try again.</p><button class="match-page__retry" type="button">Try again</button></div>`; target.querySelector(".match-page__retry")?.addEventListener("click", () => loadTab(panel)); }
    };
    ...
    app.querySelectorAll(".match-page__tab").forEach((tab) => tab.addEventListener("click", () => {
        ...
        if (panel !== "overview") loadTab(panel);
    }));

    if (match.fixtureId) {
        getFixture(match.fixtureId).then((data) => {
            const fixture = data?.response?.[0];
            if (!fixture) return;
            const details = fixture.fixture ?? {};
            app.querySelector("[data-meta='venue']").textContent = details.venue?.name ?? "Venue unavailable";
            app.querySelector("[data-meta='city']").textContent = details.venue?.city ?? "Location unavailable";
            app.querySelector("[data-meta='round']").textContent = fixture.league?.round ?? "Fixture details";
            app.querySelector("[data-meta='referee']").textContent = details.referee ? `Referee: ${details.referee}` : "Referee: unavailable";
            app.querySelector("[data-meta='date']").textContent = formatDate(details.date);
            app.querySelector("[data-meta='time']").textContent = `${formatTime(details.date)} · Asia/Tehran`;
        }).catch(() => {
            app.querySelector("[data-meta='venue']").textContent = "Venue unavailable";
            app.querySelector("[data-meta='referee']").textContent = "Referee: unavailable";
        });
    }
