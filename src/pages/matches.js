...
function normalizeFixture(fixture) {
    ...
    return {
        ...
        time: isLive ? "Live now" : new Date(fixture.fixture?.date).toLocaleTimeString("en-GB", { timeZone: TEHRAN_TIMEZONE, hour: "2-digit", minute: "2-digit" }),
...
function groupByLeague(fixtures) {
    ...
    fixtures.map(normalizeFixture).sort((a, b) => {
        const priority = getLeaguePriority(a.leagueId) - getLeaguePriority(b.leagueId);
        return priority || new Date(a.date) - new Date(b.date);
    }).forEach((match) => {
...
function createMatchesPage() {
    ...
    const loadDate = async (dateKey) => {
        ...
        try { activeFixtures = (await getMatchesByDate(getTehranDate(offsets[dateKey]))).response ?? []; cache.set(dateKey, activeFixtures); loading.hidden = true; groups.hidden = false; resultLabel.textContent = labels[dateKey]; renderCurrent(); }
        catch { loading.hidden = false; groups.hidden = true; loading.innerHTML = `<div class="matches-page__empty"><span>API ERROR</span><h2>Could not load matches</h2><p>Please check your API key and try again.</p></div>`; }
    };
